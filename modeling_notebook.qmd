...
try:
    ...
    print("⚠️ Processed data not found. Running feature engineering pipeline...")
...
print(f"Mean AUC: {tree_cv_scores.mean():.4f} ± {tree_cv_scores.std():.4f}")
print(f"\nInterpretation:")
...
baseline_results = pd.DataFrame({
    ...
    'AUC': [baseline_auc, f"{tree_cv_scores.mean():.4f} ± {tree_cv_scores.std():.4f}"],
    'Notes': [
...
print("\n✅ Baseline established. Target: AUC > 0.7 for production model")
```
...
print(f"    AUC: {lr_l2_scores.mean():.4f} ± {lr_l2_scores.std():.4f}")
...
print(f"    AUC: {lr_l1_scores.mean():.4f} ± {lr_l1_scores.std():.4f}")
...
print(f"    AUC: {lr_strong_scores.mean():.4f} ± {lr_strong_scores.std():.4f}")

print(f"\n✅ Best Logistic Regression: {max(lr_l2_scores.mean(), lr_l1_scores.mean(), lr_strong_scores.mean()):.4f}")
```
...
print(f"AUC: {rf_scores.mean():.4f} ± {rf_scores.std():.4f}")
print(f"\nInterpretation:")
print(f"  Random forest captures non-linear patterns and interactions")
print(f"  {'✅ Improvement' if rf_scores.mean() > lr_l2_scores.mean() else '⚠️ Similar performance'} over logistic regression")
```
...
print(f"AUC: {lgbm_scores.mean():.4f} ± {lgbm_scores.std():.4f}")
print(f"\nInterpretation:")
print(f"  LightGBM often excels at tabular data with complex patterns")
print(f"  {'✅ Best so far!' if lgbm_scores.mean() > max(rf_scores.mean(), lr_l2_scores.mean()) else '⚠️ Comparable to other models'}")
```
...
print(f"AUC: {xgb_scores.mean():.4f} ± {xgb_scores.std():.4f}")
print(f"\nInterpretation:")
print(f"  XGBoost balances performance with computational efficiency")
print(f"  {'✅ New best!' if xgb_scores.mean() > max(lgbm_scores.mean(), rf_scores.mean(), lr_l2_scores.mean()) else '⚠️ Similar to LightGBM'}")
```
...
print(f"\n✅ SMOTE Results:")
print(f"   AUC: {smote_scores.mean():.4f} ± {smote_scores.std():.4f}")
...
print(f"\n📊 Baseline (no SMOTE):")
print(f"   AUC: {baseline_scores.mean():.4f} ± {baseline_scores.std():.4f}")

improvement = smote_scores.mean() - baseline_scores.mean()
print(f"\n{'✅ Improvement' if improvement > 0.01 else '⚠️ Minimal impact'}: {improvement:+.4f} AUC points")
```
...
print(f"✅ Class-Weighted Model Results:")
print(f"   AUC: {weighted_scores.mean():.4f} ± {weighted_scores.std():.4f}")

improvement = weighted_scores.mean() - baseline_scores.mean()
print(f"\n{'✅ Improvement' if improvement > 0.01 else '⚠️ Minimal impact'}: {improvement:+.4f} AUC points vs. baseline")
```
...
print(f"   AUC: {undersample_scores.mean():.4f} ± {undersample_scores.std():.4f}")

improvement = undersample_scores.mean() - baseline_scores.mean()
print(f"\n{'✅ Improvement' if improvement > 0.01 else '⚠️ Worse performance'}: {improvement:+.4f} AUC points vs. baseline")
```
...
print(f"\n✅ Best Strategy: {best_strategy} (AUC: {best_imbalance_auc:.4f})")
```
...
print(f"\n📊 Data: {len(X_sample):,} rows (sample for speed)")
...
print(f"\n🎯 Best cross-validated AUC: {random_search.best_score_:.4f}")
...
print(f"\n📊 Training on full dataset: {X.shape[0]:,} rows × {X.shape[1]} features")
...
if len(supplementary_cols) > 0:
    ...
    print(f"\n📈 Model Performance:")
    print(f"   Application features only: {app_only_scores.mean():.4f} ± {app_only_scores.std():.4f}")
    ...
    print(f"\n{'✅ Significant improvement' if improvement > 0.01 else '⚠️ Minimal improvement'}: {improvement:+.4f} AUC points")
    print(f"   Supplementary data {'adds valuable predictive signal' if improvement > 0.01 else 'provides limited additional value'}")
else:
    print("\n⚠️ No supplementary features detected in dataset")
...
print("""
📊 After submitting to Kaggle, record your score here:

Public Leaderboard Score: **0.76472** ✅
Private Leaderboard Score: [Will be revealed after competition ends]
